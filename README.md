...
목적지·기간·인원을 고르면 시간대별 일정·맛집·숙소·예산을 JSON으로 짜준다.
OpenAI(gpt-4o-mini)를 먼저 쓰고 실패하면 Groq(llama-3.3)으로 넘어간다.
`.env`에 `OPENAI_API_KEY` 또는 `GROQ_API_KEY`가 있어야 한다.

**일정은 접지(RAG)해서 짠다.** 모델의 일반 지식에만 맡기면 숙소 구역·교통 요금·
시즌 특성을 그럴듯하게 지어내므로, 리포의 정제 데이터에서 그 여행에 해당하는
부분만 골라 프롬프트에 넣는다 (`backend/trip_rag.py`).

| 데이터 | 접지 내용 |
|--------|-----------|
| `calendar.json` | 날짜별 요일, 기간에 겹치는 공휴일·연휴 — 날짜 산수는 모델에 안 맡긴다 |
| `trip_profiles.json` | 시즌 특성·제철 여부·현지비 어림·성수기 배수 경고 (눈축제 기간 숙박비 1.9배 등) |
| `lodging_areas.json` | 숙박 구역 가이드 — 숙소 추천은 이 구역들 중에서만 고르게 한다 |
| `transport.json` | 구간 요금·소요시간·패스 |

데이터가 없는 목적지는 되는 부분(달력)만 접지한다. 달력에 없는 연도의 공휴일은
"없음"이 아니라 "확인 불가"로 전달한다 — 없는 정밀도를 지어내지 않는 원칙은
프롬프트에도 적용된다. 무엇을 근거로 짰는지는 결과 화면 상단에 표시된다.
