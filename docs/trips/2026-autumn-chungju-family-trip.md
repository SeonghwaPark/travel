...
| 10/30~11/1 | 🟡 **75~85%** | 5~7℃ / 17℃ | 🔴 전국 기준 **연중 최고조** | ✅ 금·토 성수기 | ★★★★☆ |
| **11/6~8** | ✅ **95~100%** | 3~6℃ / 15℃ | 🟡 여전히 높음(금요일로 상쇄) | ⚠️ 전부 동계 | **★★★★★ 채택** |
...
**③ 혼잡은 11/6이 조금 우세 — 단, 셋 중 근거가 가장 약함**

- **전국 총량 기준**: **10월 마지막 주말이 단풍 인파의 연중 정점**입니다(산 단풍 절정 + 가을 축제 시즌). 고속도로와 주요 관광지 전반은 11/6~8이 낫습니다.
- **⚠️ 그러나 청풍호 현지는 다를 수 있습니다.** 11월 초가 **호반 단풍 절정**이라는 게 이 날짜를 미는 이유인데, 그건 다른 사람들도 그때 청풍호로 온다는 뜻입니다. **청풍호반케이블카만 놓고 보면 두 날짜 차이가 거의 없거나 뒤집힐 수 있습니다.** 이 항목은 추정이며 확인된 데이터가 아닙니다.

👉 그러므로 **날짜 선택의 근거는 ①과 ④이고, 혼잡은 덤으로 취급하세요.**

**혼잡은 날짜가 아니라 구조로 이미 해결됩니다.** 금요일 출발이라 Day 1 문경은 어느 날짜든 한산합니다. 붐비는 건 **토요일 하루, 청풍호뿐**입니다. 대비책은 날짜 변경이 아니라 **09:40 도착**(10:00 오픈) — 첫 곤돌라를 타면 대기가 사실상 없습니다.
...
이 여행의 **1순위 목적은 단풍**입니다. 그러면 판단 기준은 단순해집니다. **사흘의 단풍 총량**이 최대인 날짜를 고르면 됩니다.

| 날짜 | 금 · 문경새재<br/>(산·계곡) | 토 · 청풍호<br/>(호반) | 일 · 충주호변<br/>(호반) | **평균** |
|---|---|---|---|---|
| 10/30~11/1 | 95~100% | 70~80% | 70~80% | **약 83%** |
| **11/6~8** | 70~85% | **95~100%** | **95~100%** | **약 93%** |

**사흘 중 이틀이 호수**라 호반이 절정인 쪽이 총량에서 이깁니다. 10/30을 고르면 첫날 하루만 만점이고 이틀이 미완성입니다.

**게다가 Day 1 동선이 손해를 또 줄여줍니다.** 11/6이면 문경새재 **능선**은 낙엽이 시작되지만, 우리가 실제로 걷는 **제1관문~제2관문 구간은 계곡 저지대**입니다. 저지대가 능선보다 늦게 물들기 때문에, 전동차로 올라가 걸어 내려오는 3km가 **그날 단풍이 가장 많이 남아 있는 구간**이 됩니다.

**단풍 손실이 0인 일정도 하나 있습니다.** Day 3 오전 **활옥동굴은 실내**라 단풍 시기와 무관합니다. 즉 사흘 중 야외 단풍에 노출되는 시간은 실질적으로 **금요일 종일 + 토요일 종일 + 일요일 아침**이고, 그 배분이 위 표 그대로입니다.

### 📌 결론

> ### **11월 6일(금) ~ 11월 8일(일)**
> 잃는 것은 **하루 활동시간 1시간 30분**, 얻는 것은 **호반 단풍 절정(①)** 과 **어긋남에 대한 방어(④)** 입니다.
> 혼잡(③)은 부수적 이점일 뿐이고, **11/6~8도 성수기 주말이라 한산하지는 않습니다.**

**되돌릴 조건 한 가지** — **9월 말~10월 초 산림청 단풍예측지도**가 나옵니다. 거기서 충청권이 **"평년보다 이르다"** 로 나오면 그때만 **10/30~11/1로 당기세요.** 그 경우 케이블카 09:00 오픈까지 덤으로 따라옵니다.

> 🔑 **단풍이 목적이므로, 예약할 때 이것부터 확인하세요.**
> **연수원 예약의 날짜 변경·취소가 언제까지 가능한지.** 변경이 되면 **11/6~8로 먼저 잡아두고**, 10월 초 단풍예측지도를 보고 필요할 때만 한 주 당기면 됩니다. 변경이 안 되는 조건이라면 고민 없이 **11/6~8로 확정**하세요 — 늦게 어긋날 확률이 이르게 어긋날 확률보다 크기 때문입니다.
...
| 시간 | 일정 | 메모 |
|---|---|---|
| 08:20~09:00 | 🍁 **충주댐·호반 아침 산책** | **연수원 코앞.** 11월 초 충주호는 아침 안개가 잦은데, **안개 낀 호반 단풍**이 이 여행에서 가장 예쁜 장면이 될 확률이 높습니다. 40분이면 충분 |
...
- [ ] ⭐ **연수원 예약의 날짜 변경·취소 가능 시점 확인** — 가능하면 11/6~8로 잡아두고 10월 초 단풍예측지도를 보고 조정
